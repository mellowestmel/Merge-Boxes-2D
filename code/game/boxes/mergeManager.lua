...
function Module:Merge(boxA, boxB)
    if boxA.dragging or boxB.dragging
        or boxA.merging or boxB.merging
    ...
    local velocityMagnitude = math.sqrt(
        boxA.velocityX ^ 2 + boxA.velocityY ^ 2 +
        boxB.velocityX ^ 2 + boxB.velocityY ^ 2
    ) / 2
...
function Module:CheckMerges()
    ...
    for index = 1, #boxes do
        ...
        for pointIndex = 1, #nearbyPoints do
            ...
            local canMerge = boxB and boxB ~= boxA
